...
if (isset($_POST['submit'])){
	if(isset($_FILES['image-file']) && !empty($_FILES['image-file']['name'])){
		...
		$target_dir = '../assets/img/realizations-photo/';
		...
		if ($uploadOk == 0) {
			...
			if (move_uploaded_file($_FILES["image-file"]["tmp_name"], $target_file)) {
				...
				$result = $realizacje->Create_Photo($post_cat_id, $fileName,$post_order);

				if ($result == "" || $result != FALSE){
				    echo "<script type='text/javascript'>
				            window.location.href = 'realizacje-details-{$post_cat_id}';
				          </script>";
				}
